...
const recentMatches = [
    { id: 1, player1: 'Novak Djokovic', player2: 'Rafael Nadal', score: '6-4, 6-2', status: 'completed' },
    { id: 2, player1: 'Roger Federer', player2: 'Andy Murray', score: '6-3, 4-6, 6-4', status: 'completed' },
    { id: 3, player1: 'Serena Williams', player2: 'Venus Williams', score: '3-2', status: 'live' },
    { id: 4, player1: 'Maria Sharapova', player2: 'Caroline Wozniacki', score: 'Scheduled', status: 'scheduled' }
];

const upcomingTournaments = [
    { name: 'Wimbledon Championship', date: '2024-07-01', players: 128, status: 'Upcoming' },
    { name: 'US Open Qualifier', date: '2024-07-15', players: 64, status: 'Registration' },
    { name: 'French Open Junior', date: '2024-08-01', players: 32, status: 'Planning' }
];
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="p-4 lg:p-6 space-y-4">
                        {recentMatches.map((match) => (
                            <div key={match.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  {match.status === 'live' ? '🔴 Live' :
                      match.status === 'completed' ? '✅ Done' : '⏱️ Soon'}
                </span>
                            </div>
                        ))}
                    </div>
                            ...
                            <div key={index} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition-colors">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-600">{tournament.players} players • {tournament.date}</p>
                                    </div>
                                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <button className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed border-gray-200 hover:border-green-300 hover:bg-green-50 transition-all group">
                        ...
                        <span className="text-sm font-medium text-gray-600 group-hover:text-green-700">Add Player</span>
                    </button>
                    <button className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed border-gray-200 hover:border-green-300 hover:bg-green-50 transition-all group">
                        <Trophy className="w-6 h-6 text-gray-400 group-hover:text-green-600 mb-2" />
                        <span className="text-sm font-medium text-gray-600 group-hover:text-green-700">New Tournament</span>
                    </button>
                    <button className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed border-gray-200 hover:border-green-300 hover:bg-green-50 transition-all group">
                        <Gamepad2 className="w-6 h-6 text-gray-400 group-hover:text-green-600 mb-2" />
                        <span className="text-sm font-medium text-gray-600 group-hover:text-green-700">Schedule Match</span>
                    </button>
                    <button className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed border-gray-200 hover:border-green-300 hover:bg-green-50 transition-all group">
                        <Award className="w-6 h-6 text-gray-400 group-hover:text-green-600 mb-2" />
                        <span className="text-sm font-medium text-gray-600 group-hover:text-green-700">View Results</span>
                    </button>
                </div>
            </div>
        </div>
