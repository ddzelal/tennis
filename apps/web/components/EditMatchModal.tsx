...
import { X, Trophy, Calendar, Users, Clock, AlertCircle, Save, Edit, Target, CheckCircle } from 'lucide-react';
...
export const EditMatchModal: React.FC<EditMatchModalProps> = ({
    ...
    useEffect(() => {
        if (match && isOpen) {
            // Get player IDs
            const player1Id = typeof match.player1 === 'object' ? match.player1._id : match.player1;
            const player2Id = typeof match.player2 === 'object' ? match.player2._id : match.player2;
            ...
            // Get winner ID
            const winnerId = typeof match.winner === 'object' ? match.winner._id : (match.winner || '');
    ...
    const calculateWinner = (): string => {
        const completedSets = formData.sets.filter(set =>
            set.player1Score && set.player2Score &&
            isValidTennisScore(parseInt(set.player1Score), parseInt(set.player2Score))
        );
    ...
    const renderSetInput = (setIndex: number, set: SetData) => {
        ...
        const isValidSet = set.player1Score && set.player2Score && isValidTennisScore(p1Score, p2Score);
                ...
                <div className="grid grid-cols-3 gap-4 items-center">
                    ...
                    <div className="text-center">
                        ...
                        <input
                            ...
                            onChange={(e) => updateSet(setIndex, 'player1Score', e.target.value)}
                    ...
                    <div className="text-center">
                        ...
                        <input
                            ...
                            onChange={(e) => updateSet(setIndex, 'player2Score', e.target.value)}
                ...
                {showTiebreak && (
                    <div className="mt-3">
                        ...
                        <input
                            ...
                            onChange={(e) => updateSet(setIndex, 'tiebreakScore', e.target.value)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex items-center space-x-4">
                            <div className="text-center">
                                <div className="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold">
                                    {getPlayerInitials(typeof match.player1 === 'object' ? match.player1 : null)}
                                </div>
                                <p className="text-sm font-medium mt-1">{getPlayerName(match.player1)}</p>
                            </div>
                            <div className="text-2xl font-bold text-gray-400">VS</div>
                            <div className="text-center">
                                <div className="w-12 h-12 bg-red-500 rounded-full flex items-center justify-center text-white font-bold">
                                    {getPlayerInitials(typeof match.player2 === 'object' ? match.player2 : null)}
                                </div>
                                <p className="text-sm font-medium mt-1">{getPlayerName(match.player2)}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            ...
                            <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                                match.status === MatchStatus.COMPLETED ? 'bg-green-100 text-green-800' :
                                    match.status === MatchStatus.IN_PROGRESS ? 'bg-yellow-100 text-yellow-800' :
                                        match.status === MatchStatus.CANCELLED ? 'bg-red-100 text-red-800' :
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    {/* Global Error */}
                    {(errors.general || errors.players || errors.sets || errors.winner) && (
                        <div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
                            <AlertCircle className="text-red-500 mt-0.5" size={16} />
                            <div className="flex-1">
                                {errors.general && <p className="text-red-700 text-sm">{errors.general}</p>}
                                {errors.players && <p className="text-red-700 text-sm">{errors.players}</p>}
                                {errors.sets && <p className="text-red-700 text-sm">{errors.sets}</p>}
                                {errors.winner && <p className="text-red-700 text-sm">{errors.winner}</p>}
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    {errors.player1 && (
                                        <p className="text-red-600 text-xs mt-1">{errors.player1}</p>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    {errors.player2 && (
                                        <p className="text-red-600 text-xs mt-1">{errors.player2}</p>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            {formData.status !== MatchStatus.COMPLETED ? (
                                <div className="text-center py-8">
                                    ...
                                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Match Not Completed</h3>
                                    <p className="text-gray-600 mb-4">
                                        Set the status to "Completed" in Basic Details to record results.
                                    </p>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setFormData(prev => ({ ...prev, status: MatchStatus.COMPLETED }));
                                            setActiveTab('basic');
                                        }}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {formData.sets.map((set, index) => renderSetInput(index, set))}
                                    </div>
                                        ...
                                        <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="font-medium text-green-800">
                                                    Winner: {formData.winner === formData.player1
                                                    ? getPlayerName(availablePlayers.find(p => p._id === formData.player1))
                                                    : getPlayerName(availablePlayers.find(p => p._id === formData.player2))
                                                }
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                        <h4 className="font-medium text-blue-800 mb-2">Tennis Scoring Guide:</h4>
                    ...
                    <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <button
                            type="submit"
                            disabled={updateMatchMutation.isPending || (formData.status === MatchStatus.COMPLETED && !formData.winner)}
