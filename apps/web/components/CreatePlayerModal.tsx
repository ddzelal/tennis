...
import { X, User, Calendar, Trophy, Info, CheckCircle } from 'lucide-react';
import { playersApi } from '@/lib/api/players';
import {CreatePlayerData} from "@repo/lib";
...
// Proper error interface - all fields should be strings
...
export const CreatePlayerModal: React.FC<CreatePlayerModalProps> = ({
    ...
    const createPlayerMutation = useCreatePlayer({
        ...
        onError: (error: any) => {
            ...
            if (error.response?.data?.message) {
                setErrors({
                    firstName: error.response.data.message.includes('firstName') ? error.response.data.message : undefined,
                    lastName: error.response.data.message.includes('lastName') ? error.response.data.message : undefined,
                    dateOfBirth: error.response.data.message.includes('dateOfBirth') ? error.response.data.message : undefined,
                    ranking: error.response.data.message.includes('ranking') ? error.response.data.message : undefined,
                });
            }
        }
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const submitData = {
            ...
            ...(formData.ranking && formData.ranking > 0 ? { ranking: formData.ranking } : {})
        };
    ...
    const previewName = `${formData.firstName.trim()} ${formData.lastName.trim()}`.trim();
            ...
            <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Create a new tennis player profile</p>
                    </div>
                    <button
                        onClick={handleClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-colors ${
                                errors.dateOfBirth || errors.dateInFuture || errors.ageTooYoung ? 'border-red-300 bg-red-50' : 'border-gray-300'
                            }`}
                            disabled={createPlayerMutation.isPending}
                        />
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-xs mt-1">
                            Players without ranking will be marked as "Unranked"
                        </p>
                    </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        ...
                        <ul className="text-sm text-blue-700 space-y-1">
                            <li>• Required fields: First name, Last name, Date of birth</li>
                            <li>• Players must be at least 10 years old</li>
                            <li>• Ranking is optional (1-10,000 range)</li>
                            <li>• Player profile can be updated later</li>
                        </ul>
                    </div>
                    ...
                    <div className="flex space-x-3 pt-4">
                        ...
                        <button
                            type="submit"
                            disabled={createPlayerMutation.isPending || !formData.firstName.trim() || !formData.lastName.trim() || !formData.dateOfBirth}
                    ...
                    <div className="px-6 pb-6">
                        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                            <p className="text-sm text-red-600 flex items-center">
                                ...
                                Failed to create player. Please check your input and try again.
                            </p>
                        </div>
                    </div>
                )}
