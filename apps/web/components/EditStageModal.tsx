...
export const EditStageModal: React.FC<EditStageModalProps> = ({
    ...
    const updateStageMutation = useUpdateStage({
        ...
        onError: (error) => {
            ...
            setErrors({ submit: 'Failed to update stage. Please try again.' });
        }
    });
    ...
    useEffect(() => {
        if (stage) {
            const tournamentId = typeof stage.tournament === 'object'
                ? stage.tournament._id
                : stage.tournament;
    ...
    const validateForm = () => {
        ...
        if (formData.advancingPlayers &&
            (isNaN(Number(formData.advancingPlayers)) || Number(formData.advancingPlayers) < 0)) {
            newErrors.advancingPlayers = 'Must be a valid number';
        }

        if (formData.advancingPlayers &&
            Number(formData.advancingPlayers) > formData.players.length) {
            newErrors.advancingPlayers = 'Cannot advance more players than are in the stage';
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const submitData = {
            ...
            advancingPlayers: formData.advancingPlayers ? Number(formData.advancingPlayers) : undefined,
            ...
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                    <div className="flex items-center space-x-3">
                        ...
                        <h2 className="text-xl font-semibold text-gray-900">Edit Stage</h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="w-full px-4 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-600">
                            {typeof stage.tournament === 'object' ? stage.tournament.name : 'Unknown Tournament'}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                value={formData.type}
                                onChange={(e) => setFormData(prev => ({ ...prev, type: e.target.value as StageType }))}
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData(prev => ({ ...prev, advancingPlayers: e.target.value }))}
                                placeholder="Optional"
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                            />
                            {errors.advancingPlayers && (
                                <p className="mt-1 text-sm text-red-600">{errors.advancingPlayers}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="max-h-40 overflow-y-auto border border-gray-300 rounded-lg p-3 space-y-2">
                            {players.length === 0 ? (
                                ...
                                players.map((player) => (
                                    <label key={player._id} className="flex items-center space-x-3 cursor-pointer">
                                        <input
                                            type="checkbox"
