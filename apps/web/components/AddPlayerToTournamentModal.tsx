...
export const AddPlayerToTournamentModal: React.FC<AddPlayerToTournamentModalProps> = ({
                                                                                          isOpen,
                                                                                          onClose,
                                                                                          onSuccess,
                                                                                          tournament,
    ...
    const handleAddPlayers = async () => {
        ...
        try {
            ...
            for (const playerId of selectedPlayers) {
                console.log(`🎾 Adding player ${playerId} to tournament ${tournament._id}`);
            ...
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            {tournament?.name || 'Unknown Tournament'} - {selectedPlayers.length} player(s) selected
                        </p>
                    </div>
                    <button
                        onClick={handleClose}
                ...
                <div className="p-4 bg-green-50 border-b border-gray-200">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 className="font-medium text-gray-900">{tournament?.name || 'Unknown Tournament'}</h3>
                            ...
                            {filteredPlayers.map((player) => {
                                ...
                                return (
                                    <div
                                        ...
                                        `}
                                        onClick={() => !wouldExceedLimit && togglePlayerSelection(player._id)}
                                    >
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex space-x-3">
                            ...
                            <button
                                ...
                                disabled={selectedPlayers.length === 0 || addPlayerMutation.isPending || !tournament?._id}
                                className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                            >
                                {addPlayerMutation.isPending && (
                                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                                )}
                                <span>Add {selectedPlayers.length > 0 ? `${selectedPlayers.length} ` : ''}Player{selectedPlayers.length !== 1 ? 's' : ''}</span>
                            </button>
                        </div>
                    </div>
                </div>
