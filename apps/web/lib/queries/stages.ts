...
export const stageKeys = {
  ...
  byTournament: (tournamentId: string) =>
    [...stageKeys.all, "tournament", tournamentId] as const,
};
...
export const useAddPlayerToStage = (
  options?: UseMutationOptions<
    Stage,
    Error,
    { stageId: string; playerId: string }
  >,
) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ stageId, playerId }) =>
      stagesApi.addPlayerToStage(stageId, playerId),
    onSuccess: (updatedStage, variables) => {
      // Update the specific stage in cache
      queryClient.setQueryData(
        stageKeys.detail(variables.stageId),
        updatedStage,
      );
...
export const useRemovePlayerFromStage = (
  ...
  return useMutation({
    mutationFn: ({ stageId, playerId }) =>
      stagesApi.removePlayerFromStage(stageId, playerId),
    onSuccess: (updatedStage, variables) => {
      // Update the specific stage in cache
      queryClient.setQueryData(
        stageKeys.detail(variables.stageId),
