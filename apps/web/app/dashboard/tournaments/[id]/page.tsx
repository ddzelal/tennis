...
export default function TournamentDetailsPage() {
  ...
  const [openStageDropdown, setOpenStageDropdown] = useState<string | null>(
    null,
  );

  const [activeTab, setActiveTab] = useState<
    "overview" | "players" | "stages" | "matches"
  >("overview");
  ...
  const { data: stagesResponse, refetch: refetchStages } =
    useGetStagesByTournament(tournamentId);
  ...
  const { data: matchesResponse, isLoading: isMatchesLoading } =
  ...
  const getAvailablePlayers = () => {
    ...
    return allPlayers.filter(
      (player) => !tournamentPlayerIds.includes(player._id),
    );
  };
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <span
              className={`text-xs font-medium px-2 py-1 rounded-full ${getTournamentStatusColor(tournament.status)}`}
            >
              {tournament.status}
            </span>
          </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="font-medium text-lg">
                      {tournament.settings?.setsToWin || "Not set"}
                    </p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <span className="text-sm text-gray-500">Games per Set</span>
                    <p className="font-medium text-lg">
                      {tournament.settings?.gamesPerSet || "Not set"}
                    </p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {playersCount === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">
                  Add players to start organizing matches
                </p>
                {availablePlayers.length > 0 ? (
                  <button
                    onClick={() => setIsAddPlayerModalOpen(true)}
                  ...
                  <p className="text-gray-500 text-sm">
                    No available players in the system.
                  </p>
                )}
              </div>
            ) : (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {stagesCount === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">
                  Create stages to organize your tournament
                </p>
                <button
                  onClick={() => setIsCreateStageModalOpen(true)}
                  className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors"
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <div className="flex items-center space-x-3 mb-1">
                              ...
                              <span
                                className={`px-2 py-1 text-xs font-medium rounded-full ${getStageTypeColor(stage.type)}`}
                              >
                                {getStageTypeLabel(stage.type)}
                              </span>
                              <span
                                className={`px-2 py-1 text-xs font-medium rounded-full ${getStageStatusColor(stage.status)}`}
                              >
                                {stage.status}
                              </span>
                            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {isMatchesLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">
                  Matches will appear here once they are created for this
                  tournament
                </p>
              ...
              <div className="space-y-4">
                {matches.map((match) => {
                  const player1 =
                    typeof match.player1 === "string" ? null : match.player1;
                  const player2 =
                    typeof match.player2 === "string" ? null : match.player2;
                  const winner =
                    typeof match.winner === "string" ? null : match.winner;
                  const stage =
                    typeof match.stage === "string" ? null : match.stage;
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`font-medium ${
                                winner &&
                                winner._id === (player1?._id || match.player1)
                                  ? "text-green-600"
                                  : "text-gray-900"
                              }`}
                            >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`font-medium ${
                                winner &&
                                winner._id === (player2?._id || match.player2)
                                  ? "text-green-600"
                                  : "text-gray-900"
                              }`}
                            >
