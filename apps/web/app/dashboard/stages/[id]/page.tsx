...
export default function StageDetailsPage() {
  ...
  const handleRemovePlayer = async (playerId: string) => {
    if (
      confirm("Are you sure you want to remove this player from the stage?")
    ) {
      try {
  ...
  const handleGenerateMatches = async () => {
    if (
      confirm(
        `Generate matches for this stage? This will create matches for all ${playersCount} players.`,
      )
    ) {
      try {
  ...
  const getAvailablePlayers = (): Player[] => {
    ...
    return allPlayers?.filter(
      (player) => !stagePlayerIds?.includes(player._id),
    );
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span
                  className={`px-3 py-1 text-sm font-medium rounded-full ${getStageTypeColor(stage.type)}`}
                >
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {playersCount === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">
                  Add players to get started with this stage
                </p>
                {availablePlayers.length > 0 ? (
                  <button
                    onClick={() => setIsAddPlayerModalOpen(true)}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                title={
                  playersCount < 2
                    ? "Need at least 2 players to generate matches"
